...
import { MEMBER } from "@users/urls.ts"
...
export const useMemberUpdate = () => {
    ...
    return useMutate<MemberType, MemberType>((data) => request({ method: "put", url: MEMBER, data }), {
        onSuccess: (data) => {
...
export const useMemberCreate = () => {
    ...
    return useMutate<MemberType, MemberCreateType>((data) => request({ method: "post", url: MEMBER, data }), {
        onSuccess: (data) => {
            queryClient.setQueryData<MemberType>(["member"], () => data)
        },
